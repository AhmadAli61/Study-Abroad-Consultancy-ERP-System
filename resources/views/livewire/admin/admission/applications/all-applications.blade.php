<div>
  <div class="d-flex flex-wrap justify-content-center" style="gap: 15px;">
    ...
    <div style="flex: 1 0 15%; min-width: 140px; margin: 0 4px 15px 4px;">
            ...
            <div class="card d-flex align-items-center justify-content-center text-center"
                ...
                <div class="card-body d-flex flex-column justify-content-center align-items-center p-2">
                    <div class="d-flex align-items-center gap-2">
                        ...
                        <h4 class="fw-bold m-0 text-white" style="font-size: 20px;">{{ $casReceivedCount }}</h4>
                    </div>
    ...
    <livewire:admin.admission.search-filter />
    <div class="card">
                    ...
                    <thead class="table-light">
                        <tr>
                            ...
                            <th style="vertical-align: middle; padding: 10px; white-space: normal; width: 17%;">
                                <i class="fas fa-university" style="margin-right: 5px;"></i> Uni. & Course
                            </th>
                ...
                @if(!empty($inquiry->student_name))
                    ...
                    <div style="text-align: center; margin-bottom: 10px;">
                        <strong style="font-size: 13px; color: #212529; display: block; line-height: 1.4;"
                            ...
                            @endforeach
                            @if(count($remaining))
                                ... {{-- shows dots when there’s hidden content --}}
                            @endif
                        </strong>
                    </div>
    ...
    <div >
        ...
        <div style="margin-bottom: 8px;">
            {!! !empty($inquiry->student_contact)
                ...
                : '<div style="font-size: 10px; background: #f8fafc; color: #6b7280; padding: 4px 8px; border-radius: 4px; border: 1px solid #e5e7eb; font-weight: 500;">NO CONTACT</div>' !!}
        </div>
    ...
    @php
        ...
        if (is_string($inquiry->notes_history)) {
        ...
        $notesHistory = is_array($notesHistory) ? array_reverse($notesHistory) : [];
    @endphp
...
<div class="message-bubble" style="background: #f7fafc; border-left: 3px solid {{ $userStyle['color'] }}; padding: 8px 12px; border-radius: 6px; margin-top: 6px; max-width: 100%;">
    ...
    @if ($inquiry->status === 'assigned')
        @if (!$isViewOnly)
            <span wire:click="unassignInquiry({{ $inquiry->id }})"
        ...
        @else
            <span class="badge bg-label-secondary"
                  style="font-size: 13px; padding: 4px 6px; cursor: not-allowed;"
                  title="View-only admins cannot unassign inquiries">
                {{ ucfirst($inquiry->status) }}
            </span>
        @endif
    @else
...
<td style="vertical-align: middle; padding: 8px; white-space: nowrap; text-align: center;">
    <div class="d-flex flex-column gap-1 align-items-center">
        @php
            $currentStatus = strtolower($inquiry->inquiry_status ?? 'underassessment');
        ...
        <div class="d-flex gap-1">
            {{-- Details button - Always visible for both view-only and full access --}}
            ...
            {{-- Chat button - Conditionally show based on permission --}}
            @if (!$isViewOnly)
                {{-- Full access users see the normal chat button --}}
                <button class="btn btn-sm btn-primary"
                        wire:click="$dispatch('editAdminInquiryChat', { id: {{ $inquiry->id }} })" 
                        style="font-size: 12px; padding: 3px 6px; white-space: nowrap;">
                    <i class="fas fa-comment me-1"></i> Chat
                </button>
            @else
                {{-- View-only users see a disabled chat button or alternative --}}
                <button class="btn btn-sm btn-secondary"
                        disabled
                        title="View-only admins cannot access chat"
                        style="font-size: 12px; padding: 3px 6px; white-space: nowrap; cursor: not-allowed;">
                    <i class="fas fa-comment me-1"></i> Chat
                </button>
            @endif
                        ...
                        @empty
                            <tr>
                                <td colspan="8" class="text-center text-muted">No registered inquiries found.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
            ...
            {{-- MODIFIED: Conditionally show pagination only when no filters are active --}}
    @if($registeredInquiries->count() > 0 && !$hasActiveFilters)
        <div class="card-body mt-2">
            {{ $registeredInquiries->links('pagination::bootstrap-5') }}
        </div>
    @endif

    {{-- Optional: Show a message when filters are active --}}
    @if($hasActiveFilters && $registeredInquiries->count() > 0)
        <div class="alert alert-success mt-3 container">
            <i class="fas fa-info-circle me-2"></i>
            Showing all {{ $registeredInquiries->total() }} results matching your filters. 
            Pagination is disabled when filters are active.
            <a href="javascript:void(0)" wire:click="resetFilters" class="alert-link ms-1">Clear filters</a> to enable pagination.
        </div>
    @endif
    ...
    @if($registeredInquiries->count() === 0)
        <div class="alert alert-danger mt-3 container">
            ...
            @if($hasActiveFilters)
                <a href="javascript:void(0)" wire:click="resetFilters" class="alert-link ms-1">Clear filters</a> to see all applications.
            @endif
        </div>
    @endif
    <livewire:admin.admission.modal.admin-chat-modal 
    wire:key="chat-modal-{{ time() }}"/>
    <livewire:admin.admission.modal.admin-view-modal
