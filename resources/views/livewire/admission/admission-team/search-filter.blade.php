<div>
    ...
    <div class="d-flex align-items-center gap-2">
        ...
        <!-- Reset Button -->
        <!-- In your search-filter.blade.php -->
<!-- Replace the current reset button with this: -->
...
<div class="filter-item">
    ...
    <select class="form-select form-select-sm shadow-sm" wire:model="statusFilter">
        ...
        <option value="withdrawn">Withdrawn</option> <!-- ADD THIS -->
                ...
                <!-- Date Range Picker -->
             <!-- Date Range Picker - Native HTML Solution -->
<div class="filter-item">
    <label class="form-label fw-semibold text-dark mb-2">
    ...
    <script>
        ...
        Livewire.on('loadFlatpickr', () => {
            ...
            if ($wire.tempDateRange) {
                flatpickrInstance.setDate($wire.tempDateRange);
            }
        });
        ...
        document.addEventListener('DOMContentLoaded', function() {
            Livewire.dispatch('loadFlatpickr');
        });

        // Re-initialize when Livewire updates the component
        Livewire.hook('element.updated', (el, component) => {
            if (el.id === 'flatpickr-range' && !flatpickrInstance) {
                Livewire.dispatch('loadFlatpickr');
            }
        });
    </script>
...
#flatpickr-range.form-control {
    font-size: 11px;   /* adjust to taste, e.g. 12px or 0.75rem */
}
