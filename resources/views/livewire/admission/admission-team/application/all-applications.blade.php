<div>
   <div class="d-flex flex-wrap justify-content-start" style="gap: 15px;">
    ...
    <livewire:admission.admission-team.search-filter />
    <div class="card">
                    ...
                    <thead class="table-light">
                        <tr>
                            ...
                            <th style="vertical-align: middle; padding: 10px; white-space: normal; width: 25%;">
                                <i class="fas fa-sticky-note" style="margin-right: 5px;"></i> Chat Box
                            </th>
                            <th style="vertical-align: middle; padding: 10px; white-space: nowrap; width: 8%;">
...
<div class="message-bubble" style="background: #f7fafc; border-left: 3px solid {{ $userStyle['color'] }}; padding: 8px 12px; border-radius: 6px; margin-top: 6px; max-width: 100%;">
    ...
    @if ($inquiry->status === 'assigned')
        <span onclick="unassignInquiry({{ $inquiry->id }})"
    ...
    @endif
                            ...
                            <td style="vertical-align: middle; padding: 8px; white-space: nowrap; text-align: center;">
                                ...
                                <button class="btn btn-sm btn-primary"
                                        wire:click="$dispatch('editAdmissionInquiryCommunication', { id: {{ $inquiry->id }} })" 
                                        style="font-size: 12px; padding: 3px 6px; white-space: nowrap;">
   ...
   <!-- UPDATED MODALS: Added unique keys and wire:key attributes -->
    <livewire:admission.admission-team.modals.update-admission-inquiry-communication 
        wire:key="communication-modal-{{ time() }}" />
    
    <livewire:admission.admission-team.modals.admission-inquiry-details 
        wire:key="details-modal-{{ time() }}" />
    
    <livewire:admission.admission-team.modals.modify-admission-inquiry 
        wire:key="modify-modal-{{ time() }}" />
           <livewire:admission.modal.new-admission-modal 
        wire:key="new-modal-{{ $search }}" />
    </div>
    
   <script>
function unassignInquiry(inquiryId) {
    if (!confirm('Are you sure you want to unassign this inquiry?')) {
        return;
    }

    // Show loading state
    const badge = document.querySelector(`[onclick="unassignInquiry(${inquiryId})"]`);
    const originalText = badge.textContent;
    badge.innerHTML = '<i class="fas fa-spinner fa-spin"></i>';
    badge.style.pointerEvents = 'none';

    // Make AJAX call
    fetch('{{ route("admission.unassign.inquiry") }}', {
        method: 'POST',
        headers: {
    ...
    .catch(error => {
        ...
        showToast('error', 'An error occurred while unassigning the inquiry.');
        // Restore original badge
        badge.textContent = originalText;
        badge.style.pointerEvents = 'auto';
    });
