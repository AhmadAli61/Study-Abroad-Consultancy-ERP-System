<div>
                                        ...
                                        @if($original_status === 'enrollment')
                                            ...
                                            <div class="alert alert-success py-2">
                                                <div class="d-flex align-items-center">
                                                    ...
                                                    <span>
                                                        Final Status: <strong>{{ ucwords(str_replace('_', ' ', $original_status)) }}</strong>
                                                        <span class="badge ms-2" style="background-color: #009688; color: white;">
                                                            Enrollment Completed
                                                        </span>
                                                    </span>
                                                </div>
                                                <small class="d-block mt-1">This application has reached its final status and cannot be modified.</small>
                                            </div>
                                        @else
                                            ...
                                            <div class="dropdown w-100">
                                               ...
                                               <ul class="dropdown-menu" aria-labelledby="statusDropdown" style="white-space: nowrap; padding: 5px 10px; min-width: 100%; overflow-x: auto;">
    <li class="d-flex flex-wrap gap-1" style="justify-content: start;">
        @php
            $statusColors = [
        ...
        @endforeach
                                            ...
                                            <div class="mt-2 small text-muted">
                                                <div>
                                                    ...
                                                    <span class="badge" style="background-color: {{ $statusColors[$original_status] ?? '#6c757d' }}; color: white;">
                                                ...
                                                <div class="mt-1">
                                                    ...
                                                    @foreach($this->getAllowedStatuses() as $status)
                                                        @if($status !== $original_status)
                                                            <span class="badge me-1" style="background-color: {{ $statusColors[$status] ?? '#6c757d' }}; color: white;">
                                                                {{ ucwords(str_replace('_', ' ', $status)) }}
                                                            </span>
                                                        @endif
                                                    @endforeach
    ...
    <label class="input-label">
        ...
        <input 
            type="text" 
            class="form-input @if($partner) bg-gray-100 cursor-not-allowed @endif" 
            wire:model="partner"
            @if($partner) disabled @endif
            placeholder="Enter partner name">
    </label>
    @if($partner)
        <small class="text-gray-500 text-xs">Partner information cannot be changed once set.</small>
    @endif
        ...
        <label class="file-upload-label">
            ...
            <div class="file-input-container {{ ${$field.'_path'} ? 'disabled' : '' }}">
                ...
                <div class="file-replace-overlay">
                    <span>File already uploaded - cannot be modified</span>
                </div>
                @endif
            </div>
