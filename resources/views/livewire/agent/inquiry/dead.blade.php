...
.bg-gradient-dead {
    background: linear-gradient(135deg, #4b4b4b 70%, #f8f6f6 100%) !important;
            ...
            <i class="fas fa-list-ol" style="margin-right: 5px;"></i>
                    ...
                    <tbody>
                        @php
                            $deadInquiries = $inquiries->filter(function ($inquiry) {
                                return strtolower($inquiry->inquiry_status ?? '') === 'dead';
                            });
                        @endphp
                    
                        @if ($deadInquiries->isEmpty())
                            <tr>
                        ...
                        @else
                            @foreach ($deadInquiries as $inquiry)
    ...
    <div style="border-left: 3px solid #6b7280; padding-left: 8px;">
                ...
                @if($inquiry->assigned_at)
                    <div class="assignment-info" style="text-align: center;">
                        <div class="assigned-badge" 
                             style="display: inline-flex; align-items: center; gap: 8px; padding: 8px 16px; background: #e6fffa; border-radius: 20px; font-size: 12px;">
                            <div class="status-dot" style="width: 8px; height: 8px; border-radius: 50%; background-color: #38a169;"></div>
                            <div style="display: flex; flex-direction: column; align-items: center;">
                                <span style="color: #2f855a; font-weight: 600; font-size: 11px;">
                                    Assigned: {{ \Carbon\Carbon::parse($inquiry->assigned_at)->format('M d, h:i A') }}
                                </span>
                                <span style="font-size: 10px; color: #38a169;">
...
<td style="padding: 8px; word-break: break-word; overflow-wrap: break-word; max-width: 150px;">
    ...
    <livewire:agent.modal.editInquiriesDead 
        wire:key="edit-dead-modal-{{ time() }}" />
