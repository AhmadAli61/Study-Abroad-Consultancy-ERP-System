...
.bg-gradient-cold {
    background: linear-gradient(135deg, #00bcd4 70%, #e3fcff 100%) !important;
            ...
            <i class="fas fa-list-ol" style="margin-right: 5px;"></i>
                    ...
                    <tbody>
    @php
        $coldInquiries = $inquiries->filter(function ($inquiry) {
            return strtolower($inquiry->inquiry_status ?? '') === 'cold';
        });
    @endphp

    @if ($coldInquiries->isEmpty())
        <tr>
    ...
    @else
        @foreach ($coldInquiries as $index => $inquiry)
...
<td style="padding: 8px; word-break: break-word; overflow-wrap: break-word; max-width: 150px;">
    ...
    <livewire:agent.modal.editInquiriesCold 
        wire:key="edit-cold-modal-{{ time() }}" />
