...
.bg-gradient-primary {
    ...
    <div class="response-container" 
        ...
        @if (!empty($inquiry->response))
            ...
            @php
                $followUpData = [];
                $response = trim($inquiry->response);
                
                if (!empty($response)) {
                    // Split by the F-number pattern
                    $sections = preg_split('/--- F\d+ \| [^-]+ ---/', $response);
                    preg_match_all('/--- (F\d+) \| ([^-]+) ---/', $response, $matches);
                    
                    // Handle content before first F-number (if any)
                    $initialContent = trim($sections[0] ?? '');
...
<td style="padding: 8px; word-break: break-word; overflow-wrap: break-word; max-width: 150px;">
    ...
    @if($inquiry->previous_inquiry_status || $inquiry->inquiry_status_updated_at)
        <div class="mt-1 text-center">
            ...
            <!-- Previous Status Badge (Smaller & Faded) -->
    ...
    <livewire:agent.modal.editinquiry />
    <livewire:agent.modal.viewinquiry />
