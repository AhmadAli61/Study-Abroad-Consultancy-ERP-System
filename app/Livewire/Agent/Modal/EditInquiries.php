...
class EditInquiries extends Component
{
    ...
    public function editInquiry($id)
    {
        ...
        $this->response = $inquiry->response; // Keep existing response for reference if needed
    ...
    public function update()
    {
        ...
        // If status is being changed to Registered
        if (strtolower($this->inquiry_status) === 'registered') {
            // Store the inquiry ID in session for the registration form
            session(['registering_inquiry_id' => $this->inquiryId]);
            
            // Redirect to registration form without saving yet
            return redirect()->route('agent.registered', [
                'inquiry_id' => $this->inquiryId
            ]);
        }

        // For all other status changes, update immediately using Eloquent model
    ...
    {
        return view('livewire.agent.modal.edit-inquiries');
    }
}