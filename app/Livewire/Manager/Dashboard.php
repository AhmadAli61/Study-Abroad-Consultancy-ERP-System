...
class Dashboard extends Component
{
    ...
    public function mount()
    {
        ...
        // Fetch the top 5 recent reports for the logged-in manager
    ...
    {
        $managerID = Auth::id();
        $this->totalLeads = Inquiiry::where('assigned_to', $managerID)->count();
        $this->hotLeads = Inquiiry::where('assigned_to', $managerID)->where('inquiry_status', 'hot')->count();
        $this->coldLeads = Inquiiry::where('assigned_to', $managerID)->where('inquiry_status', 'cold')->count();
        $this->deadLeads = Inquiiry::where('assigned_to', $managerID)->where('inquiry_status', 'dead')->count();
        $this->pendingLeads = Inquiiry::where('assigned_to', $managerID)->where('inquiry_status', 'pending')->count();
        $this->registeredLeads = Inquiiry::where('assigned_to', $managerID)->where('inquiry_status', 'registered')->count();
    }
    ...
    protected function loadApplicationCounts()
    {
        ...
        // Base query for counts - only show applications for this manager
        $query = RegisteredInquiry::where('users_id', $managerID);
        ...
        $this->withdrawnCount = $query->clone()->where('inquiry_status', 'withdrawn')->count();
    }
    ...
    protected function loadDashboardData()
    {
        ...
        $this->recentInquiries = Inquiiry::where('assigned_to', $managerID)
            ->orderBy('created_at', 'desc')
            ->take(5)
            ->get();

        // Recent applications (last 5)
        $this->recentApplications = RegisteredInquiry::where('users_id', $managerID)
            ->orderBy('created_at', 'desc')
            ->take(5)
            ->get();

        // Application status distribution
        $this->statusDistribution = RegisteredInquiry::where('users_id', $managerID)
        ...
        $this->inquiryStatusDistribution = Inquiiry::where('assigned_to', $managerID)
            ->select('inquiry_status', DB::raw('count(*) as count'))
            ->groupBy('inquiry_status')
            ->get()
            ->toArray();
        ...
        $this->topUniversities = RegisteredInquiry::where('users_id', $managerID)
