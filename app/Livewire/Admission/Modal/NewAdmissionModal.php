...
class NewAdmissionModal extends Component
{
    use WithFileUploads;
    ...
    #[On('openNewAdmissionModal')]
    public function openNewAdmissionModal($parentId)
    {
        ...
        session(['new_admission_referrer' => url()->previous()]);
        $this->showModal = true;
    ...
    public function saveNewAdmission()
    {
        ...
        try {
            ...
            // Create new admission linked to parent - WITHOUT unique_id first
            $newAdmission = RegisteredInquiry::create([
            ...
            $uniqueId = $this->generateUniqueId($parent->student_name, $newAdmission->id);
            ...
            $this->closeModal();
            session()->flash('success', 'New admission added successfully! Registration ID: ' . $uniqueId);
            
            // Get the referrer from session
            $referrer = session('new_admission_referrer', route('admissionagent.allapplications'));
            
            return redirect()->to($referrer);

        } catch (\Exception $e) {
            session()->flash('error', 'Error creating new admission: ' . $e->getMessage());
        }
    }

    public function render()
    {
        return view('livewire.admission.modal.new-admission-modal');
    }
}