...
class UnconditionalOffers extends Component
{
    ...
    public $search = '';
    public $statusFilter = 'unconditional'; // Default to unconditional status
 ...
 protected function loadCounts()
{
    ...
    {
        // Get the current authenticated agent's ID
         $accessibleUserIds = UserHelper::getAccessibleUserIds(auth()->user());
        ...
        $query = RegisteredInquiry::query()
            ...
            ->where('inquiry_status', 'unconditional') // Filter for unconditional status
        ...
        return view('livewire.admission-agent.applications.unconditional-offers', [
