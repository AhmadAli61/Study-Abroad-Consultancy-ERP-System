...
use App\Helpers\UserHelper; // Add this import
   ...
   protected function loadCounts()
{
    ...
    public function render()
    {
        ...
        $query = RegisteredInquiry::query()
            ->whereIn('assigned_to', $accessibleUserIds) // Show inquiries assigned to agent OR their parent/assistant
            ->where('status', '!=', 'unassigned')
            ->whereNull('intake_id')
            ->where('inquiry_status', '!=', 'withdrawn') // Exclude withdrawn from counts
            ->where('inquiry_status', '!=', 'rejection') // Exclude rejection status
        ...
        return view('livewire.admission-agent.applications.all-applications', [
