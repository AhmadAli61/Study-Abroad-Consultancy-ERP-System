...
class ConditionalOffersForm extends Component
{
    ...
    public $hasExistingPortalLogins = false; // Add this property

    protected $rules = [
        'conditional_document' => 'nullable|file|mimes:pdf|max:5120',
        'applicationPortalLogins' => 'required|string|max:255',
    ];

    protected $messages = [
        'applicationPortalLogins.required' => 'The application portal logins field is required.',
    ...
    public function mount($inquiry_id)
    {
        ...
        if ($this->admissionForm) {
            ...
            $this->hasExistingPortalLogins = !empty($this->admissionForm->application_portal_logins); // Set this flag
        }
    }
    ...
    public function submitForm()
    {
        ...
        // Only validate application portal logins if they don't exist already
        if (!$this->hasExistingPortalLogins) {
            $this->validate([
                'applicationPortalLogins' => 'required|string|max:255',
            ]);

            // Custom validation for application portal logins format
        ...
        // Only update application portal logins if they don't exist already
        if (!$this->hasExistingPortalLogins) {
            $formData['application_portal_logins'] = $this->applicationPortalLogins;
        }
    ...
    {
        return view('livewire.admission-agent.admission-forms.conditional-offers-form')->layout('layouts.admissionagentdashboard');
    }
}