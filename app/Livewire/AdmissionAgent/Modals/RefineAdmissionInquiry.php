...
class RefineAdmissionInquiry extends Component
{
    use WithFileUploads;
    ...
    public function editAdmissionInquiry($id)
    {
    ...
    public function getAllowedStatuses()
    {
        ...
        if ($currentIndex !== false && $currentIndex < count($this->statusFlow) - 2) {
    ...
    public function update()
    {
        ...
        if ($this->inquiry_status === 'processed' && $this->inquiry_status !== $this->original_status) {
            ...
            $admissionForm = AdmissionForm::where('registered_inquiry_id', $inquiry->id)
                ...
                ->whereNotNull('application_submission')
                ->first();
            ...
            if (!$admissionForm) {
                ...
                return redirect()->route('admissionagent.forms.processed-form', ['inquiry_id' => $inquiry->id])
        ...
        if ($this->inquiry_status === 'conditional' && $this->inquiry_status !== $this->original_status) {
            ...
            $admissionForm = AdmissionForm::where('registered_inquiry_id', $inquiry->id)
                ...
                ->whereNotNull('conditional_document')
                ->first();
            ...
            if (!$admissionForm) {
                ...
                return redirect()->route('admissionagent.forms.conditional-offers-form', ['inquiry_id' => $inquiry->id])
        ...
        if ($this->inquiry_status === 'unconditional' && $this->inquiry_status !== $this->original_status) {
            ...
            $admissionForm = AdmissionForm::where('registered_inquiry_id', $inquiry->id)
                ->whereNotNull('extra_undercas_path') // Changed from fee_voucher_path
            ...
            if (!$admissionForm) {
                ...
                return redirect()->route('admissionagent.forms.unconditional-offers-form', ['inquiry_id' => $inquiry->id])
        ...
        if ($this->inquiry_status === 'undercas' && $this->inquiry_status !== $this->original_status) {
            ...
            if (!$admissionForm) {
                ...
                return redirect()->route('admissionagent.forms.under-cas-form', ['inquiry_id' => $inquiry->id])
        ...
        if ($this->inquiry_status === 'casreceived' && $this->inquiry_status !== $this->original_status) {
            ...
            $admissionForm = AdmissionForm::where('registered_inquiry_id', $inquiry->id)
                ...
                ->first();
                                    $interviewPassRequired = !$admissionForm || !$admissionForm->interview_pass_path;

            
    if (!$admissionForm || $interviewPassRequired) {
                // First update all other changes except status
                unset($updateData['inquiry_status']);
                ...
                return redirect()->route('admissionagent.forms.cas-received-form', ['inquiry_id' => $inquiry->id])
                             ->with('status_message', 'Please upload CAS document' . ($interviewPassRequired ? ' and interview confirmation' : '') . ' before changing status to CAS Received');
    }
        }
        ...
        if ($this->inquiry_status === 'visaprocess' && $this->inquiry_status !== $this->original_status) {
            ...
            if (!$admissionForm) {
                ...
                return redirect()->route('admissionagent.forms.visa-process-form', ['inquiry_id' => $inquiry->id])
        ...
        if ($this->inquiry_status === 'enrollment' && $this->inquiry_status !== $this->original_status) {
            ...
            if (!$admissionForm) {
                ...
                return redirect()->route('admissionagent.forms.enrollment-form', ['inquiry_id' => $inquiry->id])
    ...
    {
        return view('livewire.admission-agent.modals.refine-admission-inquiry');
    }

   

    // Add this to your RefineAdmissionInquiry component
