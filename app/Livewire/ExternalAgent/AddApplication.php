...
class AddApplication extends Component
{
    ...
    // File upload properties (same as Registered.php)
...
{
    // First check if phone number already exists in inquiries table
    $existingInquiry = Inquiiry::where('phone_number', $this->student_contact)->first();
    ...
    // Use the same validation rules as Registered.php
    $this->validate([
    ...
    try {
        // 1. First create a minimal inquiry record based on inquiiries table structure
        $inquiry = Inquiiry::create([
            'name' => $this->student_name,
            'email' => $this->extractEmailFromGmailPassword(), // Extract email from gmail_password field
            'phone_number' => $this->student_contact,
            'type' => 'External Agent Direct', // Custom type for external agent direct applications
            'status' => 'assigned',
            'assigned_to' => Auth::id(),
            'assigned_at' => now(),
            'inquiry_status' => 'registered', // Set directly to registered since it's a direct application
            'study_course' => $this->course_name,
            'country' => 'N/A', // Or you can add a country field to your form
            'response' => 'Direct application from external agent portal',
            'website' => 'external_agent_portal',
            'source' => 'external_agent_direct', // Custom source identifier
        ]);

        // 2. Now store all files (same as Registered.php)
        ...
        $studentConsentPath = $this->student_consent ? $this->student_consent->store('registered-docs') : null;
        ...
        // 3. Create the registered inquiry using the new inquiry_id
        $registeredInquiry = RegisteredInquiry::create([
            'inquiry_id' => $inquiry->id, // Use the newly created inquiry ID
            'users_id' => auth()->id(),
            ...
            'reference_letters_path' => $referenceLetterPath,
        ...
        // 4. Generate unique ID and update
        $uniqueId = $this->generateUniqueId($this->student_name, $registeredInquiry->id);
        $registeredInquiry->update(['unique_id' => $uniqueId]);

        // 5. Create notification
        Notification::create([
            'type' => 'new_application',
            'notifiable_type' => 'admission_team',
            'notifiable_id' => 0, 
            ...
            'data' => [
                ...
                'source' => 'external_agent_direct'
            ]
        ]);
                $this->reset();

        session()->flash('success', 'Application submitted successfully! Registration ID: ' . $uniqueId);
        return redirect()->route('externalagent.all-applications');
    ...
    // Helper method to extract email from gmail_password field
    private function extractEmailFromGmailPassword()
    {
        if ($this->gmail_password) {
            $parts = explode('/', $this->gmail_password);
            return trim($parts[0]) ?? null;
        }
        return null;
    }

    // Copy the same helper methods from Registered.php
    ...
    {
        return view('livewire.external-agent.add-application')->layout('layouts.externalagent');
    }
}