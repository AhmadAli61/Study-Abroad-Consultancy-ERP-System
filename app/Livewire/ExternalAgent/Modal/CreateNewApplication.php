...
class CreateNewApplication extends Component
{
      use WithFileUploads;
    ...
    public function openAddNewApplicationModal($parentId)
    {
        $this->parentId = $parentId;
            // Get the parent inquiry to identify the student
    $parentInquiry = RegisteredInquiry::findOrFail($parentId);
      if ($parentInquiry) {
    ...
    if ($existingApplicationsCount >= 3) {
        ...
        try {
            ...
            $referrer = session('new_application_referrer', route('externalagent.all-applications'));
    ...
    {
        return view('livewire.external-agent.modal.create-new-application');
    }
}
